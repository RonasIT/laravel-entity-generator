...
The `--methods` option no longer falls back to its `CRUD` default when `--only-entity` is passed. Commands
relying on the previous behaviour have to pass the methods explicitly:

```bash
php artisan make:entity Post --only-entity --methods=CRUD
```

The same applies to `--only-entity` combined with other `--only-*` options: without an explicit `--methods`
the controller, resource and request generators of that run receive an empty set of methods.
