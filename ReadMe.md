...
Every declared relation is also added to the generated API resource as a nested resource, so the foreign key column
...
option. A field whose name matches a relation foreign key (e.g. `-i user_id` together with `-e User`) is omitted from
the resource in favor of the nested relation.

Resources of the related entities are never generated by the command, therefore they have to exist in the project
before the generation:

| Relation type | Required resource |
| -------- | ------- |
...
```
Cannot create PostResource cause UserResource does not exist. Create app/Http/Resources/User/UserResource.php and run command again.
```
